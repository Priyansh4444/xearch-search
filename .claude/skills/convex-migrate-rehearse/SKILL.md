...
description: "Rehearse a live-app schema change + backfill on a snapshot-seeded preview deployment, verify, then promote the proven change to prod with the snapshot as a data-recovery input."
---
...
1. BRANCH before changing schema or migration code: create and switch to a dedicated migration branch unless the user explicitly directed otherwise.
2. GUARD: deploy-guard — classify + announce the SOURCE (prod, being read) and the eventual TARGET (prod, being changed), and confirm the plan. Consent for reading/exporting does not authorize promotion.
3. SNAPSHOT the production source data read-only: `npx convex export --prod --path snapshot.zip`, or use `--deployment <source-deployment>` when production is selected explicitly by name/reference (add `--include-file-storage` only if the migration touches files). Record the exact source deployment with the artifact. This is a read; it changes nothing.
...
7. PROMOTE: immediately before changing production, announce the exact target and sequence and obtain fresh explicit consent through deploy-guard. Then apply the SAME sequence to prod (optional schema → backfill → tighten). Because it already succeeded on prod-shaped data, the prod push repeats a proven run. Treat the snapshot as one data-recovery input, not a complete rollback: `--replace` restores only imported tables, does not restore deployed schema/functions, and leaves data outside those tables. Roll back failed code/schema separately. Newly created tables need an explicit tested rollback plan. `--replace-all` may delete data and requires confirmation before use.
...
- Snapshot import is not a complete rollback. `--replace` restores only imported tables; it does not restore deployed schema/functions, and other data may remain. Roll back code separately, explicitly plan newly created tables, and require confirmation before destructive `--replace-all`.
