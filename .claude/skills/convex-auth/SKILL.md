...
2. Choose the provider explicitly. Use Passkey only when the user chooses it; otherwise default to another supported provider such as Password, magic link/OTP, or OAuth. Add the selected provider in convex/auth.ts.
...
- Do not make Passkey implicit. Require an explicit Passkey choice, or use a supported Password, magic link/OTP, or OAuth provider.
