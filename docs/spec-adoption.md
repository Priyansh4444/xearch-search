...
3. Authenticated downstream publication updates, distinct from raw capture receipts. Durable long imports should move to a worker-side job API with bounded Convex reconciliation; the current collector performs bounded 500-post acquisitions, not one action for an entire account archive.
