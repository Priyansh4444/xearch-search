...
const client = new ConvexHttpClient(process.argv.includes("--production") ? "https://utmost-kudu-321.convex.cloud" : "http://127.0.0.1:3210");
const auth = await client.action("auth:signIn", { provider: "anonymous" });
if (!auth.tokens?.token)
  throw new Error("Local guest authentication did not return a token.");
...
let last;
for (let i = 0; i < 70; i++) {
  const job = (await client.query("jobs:list", {})).find(
    (job) => job._id === jobId,
  );
  ...
  if (!["queued", "running"].includes(job.status)) {
    if (job.status !== "complete")
      throw new Error(job.error ?? "Capture did not complete");
