...
4. MCP safety defaults: identify the actual non-production deployment name/reference, then start the official MCP with `--deployment <non-production-deployment>` (for example the resolved personal-dev deployment name). Do not rely on a generic/default-style selector. The two prod flags are DIFFERENT risk levels — keep them split: a read-only prod audit (advisor/insights reading data/logs/insights) passes ONLY `--cautiously-allow-production-pii` (read tools); `--dangerously-enable-production-deployments` (which enables MUTATING prod tools) stays OFF unless the user explicitly asked to CHANGE prod this session. Never pair them by default — 'look at prod' must not silently grant 'mutate prod'.
