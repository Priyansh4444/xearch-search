...
3. NORMALIZE + DEDUPE: collect all findings into one report. Build finding identity from both (a) the normalized code/deployment locus, such as `messages:list`, and (b) the defect resource, such as the violated rule, affected field, index, or operation. Equivalent reports of the same defect collapse even when one uses a code locus and another a deployment locus; distinct defects on the same function/table remain separate. Keep the higher-confidence source. Drop nothing silently; a pass that errored/was skipped is a stated coverage gap, not a clean result.
...
- Dedup by normalized locus plus defect resource. Map deployment functionId ↔ code file:line so equivalent reports collapse, while different violated rules, fields, indexes, or operations on the same locus remain distinct.
