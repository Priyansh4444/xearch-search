...
- The exact import table — application function builders (`query`/`mutation`/`action`/`internalQuery`/`internalMutation`/`internalAction`) come from `"./_generated/server"`; `api`/`internal` come from `"./_generated/api"`. Framework exports such as `defineSchema`, `defineTable`, `cronJobs`, and supported validators/schema helpers legitimately come from `"convex/server"`; application function builders do not.
