...
4. Attach the domain through Convex Deployment Settings or the supported management API, then wait for verification; do not invent a dedicated Convex CLI domain-attach command. DNS propagation can take minutes to hours. Verify records landed with `dig +short`.
