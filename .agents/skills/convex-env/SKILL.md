...
2. Set secrets without putting values in CLI arguments: use interactive input/stdin or `npx convex env set KEY --from-file value.txt`. For multiple values, create a dedicated allowlisted file such as `.env.convex` containing only the required Convex variables, then use `--from-file .env.convex`. Never upload the project's full `.env`; exclude `CONVEX_DEPLOY_KEY` and unrelated credentials.
...
- Never upload a whole project `.env`; use a dedicated allowlisted file containing only the intended Convex variables and no deploy key.
